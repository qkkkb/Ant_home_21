...
3. 无线遥控 `CH7`：当前禁用，因为无线串口用于向从车发送闭环运动反馈。
...
整体行为是先搜索目标，再粗对准、细对准、分类推送方向、必要时环绕调整姿态，然后执行推送、越过黄线、后退、转回和后续前进。主车不再等待从车协同推物体；从车只根据红外灯板和无线闭环运动反馈持续跟随。
...
主车使用当前已验证的无线帧结构向从车发送闭环运动反馈。无线参数位于 `config.py`：
...
该消息 payload 使用定点 `int16` 发送 `vx*10`、`vy*10`、`wz*10`、`yaw*10`，并带 1 字节 flags。`vx/vy` 由主车编码器闭环速度反算得到，`wz` 使用陀螺仪实际角速度，闭环反馈帧会置位 `MASTER_MOTION_FLAG_CLOSED_LOOP`；从车按该反馈叠加红外视觉闭环，提高跟随响应速度和卡顿时的稳定性。
