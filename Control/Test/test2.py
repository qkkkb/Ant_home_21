    ...
    while True:
        ...
        if pit_flag:
            ...
            if pit_count % DEBUG_DIV == 0 and RUN_MODE == "wheel_dir_cal":
                ...
                if ENABLE_IMU:
                    log(
                        "[SPEED LOOP] t=%.2fs enc=(%d,%d,%d) tar=(%.1f,%.1f,%.1f) out=(%.1f,%.1f,%.1f) pwm=(%d,%d,%d) raw_gz=%.1f gyro=%.2f yaw=%.2f yaw_err=%.2f vz=%.2f"
                        % (
                            elapsed_s,
                            int(snap["enc_fl"]),
                            int(snap["enc_fr"]),
        ...
        if utime.ticks_diff(now, last_status_ms) >= 1000:
            ...
            if RUN_MODE == "wheel_dir_cal":
                ...
                if ENABLE_IMU:
                    log(
                        "[STATUS] t=%.1fs ch7=%.1f raw_gz=%.1f gyro=%.2f yaw=%.2f yaw_err=%.2f vz=%.2f"
                        % (
                            elapsed_s,
                            ch7_init_value,
