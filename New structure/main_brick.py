...
from move_base import calc_wheel_spd as _base_calc_wheel_spd
...
Red_Brick_Packet_Timeout_Ms = 250
Red_Brick_Image_Center_X = 96
Cam_Frame_Head = 0xFF
...
NAV_STATE_SEARCH = 0
NAV_STATE_SEARCH_TURN = 1
NAV_STATE_COARSE = 2
NAV_STATE_FINE = 3
NAV_STATE_PUSH_CLASSIFY = 4
NAV_STATE_PUSH_ORIENT = 5
NAV_STATE_PUSH_PREPARE = 6
NAV_STATE_PUSH = 7
NAV_STATE_PUSH_BACK = 8
NAV_STATE_PUSH_TURN = 9
NAV_STATE_POST_TURN_FORWARD = 10
NAV_STATE_RETURN_LEFT = 11
NAV_STATE_RETURN_BACK = 12
NAV_STATE_RETURN_TURN = 13
...
ART_MODE_FOLLOW_FINE_CMD = b"FOLLOW_FINE\n"
ART_MODE_FINE_CMD = b"FINE\n"
ART_MODE_CLASSIFY_CMD = b"CLASSIFY\n"
ART_MODE_PUSH_CMD = b"PUSH\n"
...
Nav_Fine_Forward_Limit = 14.0
Nav_Fine_Lateral_Limit = 14.5
Nav_Fine_Push_Lateral_Limit = 8.5
...
Nav_Fine_Push_Lateral_Brake_Speed = 3.0
...
Nav_Push_Prepare_Ok_Ms = 40
Nav_Push_Prepare_Target_X = 10
...
Nav_Ball_Push_Yaw_Offset = 15.0
Nav_Bear_Push_Execute_Forward_Speed = 20.0
Nav_Bear_Push_Ms = 2400
Nav_Ball_Field_Vx_Scale = 0.9659258
Nav_Ball_Field_Vy_Scale = 0.2588190
...
Nav_Return_Final_Back_Speed = 20.0
Nav_Return_Final_Line_Extra_Ms = 100
Nav_Return_Turn_Ok_Yaw = 10
Nav_Return_Turn_Ok_Ms = 0
Nav_Return_Max_Ms = 6000

Launch_Forward_Ms = 1500
Launch_Shift_Right_Ms = 1000
Launch_Forward_Speed = 22.0
Launch_Shift_Right_Speed = 13.0

Red_Brick_Trigger_Bottom_Y = 40
Red_Brick_Left_Clear_Raw_X = 65
Red_Brick_Right_Clear_Raw_X = -78
Red_Brick_Shift_Forward_Scale = 0.70
Red_Brick_Shift_Lateral_Scale = 0.60
Red_Brick_Pass_Forward_Scale = 0.85
Red_Brick_Push_Start_Guard_Ms = 500

# ====================== 全局状态变量 ======================
# 小车启动标志：False=上电静止，True=已启动
car_started = False
auto_start_done = False
launch_motion_phase = 0
launch_motion_start_ms = 0
# 上一次 C9 发车键状态（用于消抖）
last_c9_state = 1
# 上一次 C8 退出键状态（用于消抖）
last_c8_state = 1
...
red_brick_has_target = False
red_brick_raw_x = 0
red_brick_bottom_y = 0
red_brick_last_rx_ms = 0
red_brick_avoid_state = 0
...
def clear_red_brick_state():
    global red_brick_has_target, red_brick_raw_x, red_brick_bottom_y
    global red_brick_last_rx_ms
...
def update_red_brick_packet(flags, mid_x, bottom_y):
    global red_brick_has_target, red_brick_raw_x, red_brick_bottom_y
    global red_brick_last_rx_ms
    global nav_target_lost_since_ms, nav_fine_ok_since_ms, nav_transition_ms

    red_brick_last_rx_ms = utime.ticks_ms()
    if flags & 0x01:
        red_brick_has_target = True
        red_brick_raw_x = int(mid_x) - Red_Brick_Image_Center_X
        red_brick_bottom_y = int(bottom_y)
        if red_brick_bottom_y >= Red_Brick_Trigger_Bottom_Y:
            nav_target_lost_since_ms = 0
            if nav_state == NAV_STATE_FINE and not push_orbit_done:
                nav_fine_ok_since_ms = 0
            if nav_state in (NAV_STATE_SEARCH_TURN, NAV_STATE_POST_TURN_FORWARD):
                nav_transition_ms = red_brick_last_rx_ms
    else:
        red_brick_has_target = False
        red_brick_raw_x = 0
        red_brick_bottom_y = 0
...
def red_brick_target_seen():
    if not red_brick_has_target or red_brick_last_rx_ms == 0:
        return False
    return (
        utime.ticks_diff(utime.ticks_ms(), red_brick_last_rx_ms)
        <= Red_Brick_Packet_Timeout_Ms
    )


def calc_wheel_spd(move, vx, vy, vz):
    global red_brick_avoid_state, cam_target_vx, cam_target_vy
    global nav_target_lost_since_ms, nav_fine_ok_since_ms

    enabled = launch_motion_phase == 0 and (
        nav_state in (
            NAV_STATE_SEARCH_TURN,
            NAV_STATE_COARSE,
            NAV_STATE_PUSH,
        ...
        or (nav_state == NAV_STATE_FINE and not push_orbit_done)
    )
    if not enabled:
        red_brick_avoid_state = 0
    else:
        push_start_guard = (
            nav_state == NAV_STATE_PUSH
            and push_dir_code == Push_Dir_Left
            and utime.ticks_diff(utime.ticks_ms(), nav_transition_ms)
            < Red_Brick_Push_Start_Guard_Ms
        )
        if push_start_guard:
            red_brick_avoid_state = 0
        else:
            seen = red_brick_target_seen()
            if red_brick_avoid_state == 0:
                if seen and red_brick_bottom_y >= Red_Brick_Trigger_Bottom_Y:
                    if red_brick_raw_x >= Red_Brick_Left_Clear_Raw_X:
                        red_brick_avoid_state = 3
                    else:
                        red_brick_avoid_state = 1
            elif not seen:
                red_brick_avoid_state = 0
            elif (
                red_brick_avoid_state == 1
                and red_brick_raw_x <= Red_Brick_Right_Clear_Raw_X
            ):
                red_brick_avoid_state = 2

        if red_brick_avoid_state:
            nav_target_lost_since_ms = 0
            if nav_state == NAV_STATE_FINE:
                nav_fine_ok_since_ms = 0

        if red_brick_avoid_state == 1:
            base_speed = abs(vx)
            vx *= Red_Brick_Shift_Forward_Scale
            vy = base_speed * Red_Brick_Shift_Lateral_Scale
        elif red_brick_avoid_state == 2:
            vx *= Red_Brick_Pass_Forward_Scale
            if vy < 0.0:
                vy = 0.0
        elif red_brick_avoid_state == 3:
            vx *= Red_Brick_Pass_Forward_Scale
            if vy > 0.0:
                vy = 0.0
    ...
    move.tar_spd_x = vx
    move.tar_spd_y = vy
    _base_calc_wheel_spd(move, vx, vy, vz)
...
def send_art_mode_command(new_state):
    if new_state in (NAV_STATE_SEARCH, NAV_STATE_SEARCH_TURN, NAV_STATE_SEARCH_SPIN, NAV_STATE_POST_TURN_FORWARD):
        ...
        if push_orbit_done:
            ...
            cam_uart.write(ART_MODE_FOLLOW_FINE_CMD)
        ...
        cam_uart.write(ART_MODE_PUSH_CMD)
...
def nav_state_code(state):
    if (
        (state == NAV_STATE_SEARCH_SPIN and final_return_mode == 3)
        or (state == NAV_STATE_RETURN_DONE and final_return_mode == 4)
    ):
        return 17
    return state


def nav_set_state(new_state, force=False):
    ...
    global yaw_ref_deg, cam_rx_started, push_dir_code
    ...
    if new_state == NAV_STATE_SEARCH and final_return_mode == 3:
        new_state = NAV_STATE_RETURN_DONE
        final_return_mode = 4
    ...
    if new_state == NAV_STATE_PUSH_CLASSIFY:
        ...
        if final_return_mode == 2:
            line_crossed = False
    elif new_state == NAV_STATE_RETURN_DONE:
        if final_return_mode != 4:
            final_return_mode = 3
    ...
    elif new_state == NAV_STATE_RETURN_FINAL:
        yaw_ref_deg = normalize_yaw_deg(field_up_yaw - Nav_Return_Turn_Ok_Yaw)
    elif new_state == NAV_STATE_RETURN_DONE:
        yaw_ref_deg = field_up_yaw
...
def apply_nav_targets(vx, vy, vx_limit, vy_limit, lateral_error=None):
    global cam_target_vx, cam_target_vy

    if lateral_error is None:
        lateral_error = cam_error_x
    if -Nav_Forward_Deadband <= cam_error_y <= Nav_Forward_Deadband:
        vx = 0.0
    if -Nav_Lateral_Deadband <= lateral_error <= Nav_Lateral_Deadband:
        vy = 0.0
...
def begin_launch_motion(now):
    global launch_motion_phase, launch_motion_start_ms

    launch_motion_phase = 1
    launch_motion_start_ms = now
    nav_set_state(NAV_STATE_SEARCH_TURN, force=True)
...
def update_return_home(now, yaw_deg, low_speed, gyro_z):
    ...
    if nav_state == NAV_STATE_RETURN_DONE:
        ...
        cam_target_vx = 0.0
        cam_target_vy = 0.0
        if final_return_mode == 3 and utime.ticks_diff(now, nav_transition_ms) >= 1000:
            nav_set_state(NAV_STATE_SEARCH_SPIN)
        return
...
def update_nav_state_and_targets(yaw_deg, low_speed, prepare_low_speed, gyro_z):
    ...
    now = utime.ticks_ms()
    if launch_motion_phase:
        nav_ready_for_push = False
        yaw_ref_deg = field_up_yaw
        elapsed_ms = utime.ticks_diff(now, launch_motion_start_ms)
        if launch_motion_phase == 1:
            cam_target_vx = Launch_Forward_Speed
            cam_target_vy = 0.0
            if elapsed_ms >= Launch_Forward_Ms:
                launch_motion_phase = 2
                launch_motion_start_ms = now
                reset_speed_pid_state()
        else:
            cam_target_vx = 0.0
            cam_target_vy = Launch_Shift_Right_Speed
            if elapsed_ms >= Launch_Shift_Right_Ms:
                launch_motion_phase = 0
                cam_target_vx = 0.0
                cam_target_vy = 0.0
                reset_speed_pid_state()
    ...
    if nav_state == NAV_STATE_SEARCH_SPIN:
        ...
        if push_orbit_progress_deg >= 360.0 - Nav_Push_Turn_Ok_Yaw:
            nav_set_state(NAV_STATE_SEARCH)
        ...
        if (
            ...
            if nav_fine_ok_since_ms == 0:
                ...
                if push_orbit_done:
                    ...
                    nav_set_state(NAV_STATE_PUSH_CLASSIFY)
        else:
            nav_fine_ok_since_ms = 0
        return
    ...
    if nav_state == NAV_STATE_PUSH:
        ...
        if push_dir_code == Push_Dir_Right:
            cam_target_vx = Nav_Bear_Push_Execute_Forward_Speed
            cam_target_vy = 0.0
            yaw_ref_deg = push_yaw_target
            if utime.ticks_diff(now, nav_transition_ms) >= Nav_Bear_Push_Ms:
                nav_set_state(NAV_STATE_PUSH_BACK)
            return
        cam_target_vx = (
...
def poll_art_uart():
    ...
    global push_dir_code, push_yaw_target, line_crossed
        ...
        if data:
            ...
            while len(cam_rx_buf) >= 3:
                if cam_rx_buf[0] != Cam_Frame_Head:
                    ...
                    continue
                if cam_rx_buf[1] == Red_Brick_Packet_Tag:
                    if len(cam_rx_buf) < 5:
                        break
                    update_red_brick_packet(
                        int(cam_rx_buf[2]),
                        int(cam_rx_buf[3]),
                        int(cam_rx_buf[4]),
                    )
                    cam_rx_buf = cam_rx_buf[5:]
                    continue
    ...
    if current_c9 == 0 and last_c9_state == 1:
        ...
        if key_start.value() == 0:
            if not car_started:
                ...
                begin_launch_motion(start_time)
