...
class MoveBasePolarityTest:
    ...
    def help(self):
        self.log("cmd: F/B/L/R [spd] [ms]")
        self.log("cmd: CW/CCW [spd] [ms]")
        self.log("cmd: MOVE vx vy vz [ms]")
        self.log("cmd: M FL|FR|B duty [ms]")
        self.log("cmd: STOP, HELP")
    ...
    def apply_single_motor(self, role, duty, run_ms):
        ...
        self.log(
            "M %s pwm=%d,%d,%d ms=%d"
            % (role, apply_min_duty(pwm_fl), apply_min_duty(pwm_fr), apply_min_duty(pwm_b), int(run_ms))
        )
    ...
    def run(self):
        self.log("=== move_base polarity wireless test ===")
        self.log(
        ...
        try:
            while True:
                ...
                if utime.ticks_diff(now, self.last_status_ms) >= STATUS_PERIOD_MS:
                    ...
                    self.log("READY active=%d" % (1 if self.active else 0))
