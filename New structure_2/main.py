...
Follow_Static_Visual_Scale = 0.75
...
Follow_Pose_Wheel_Target_Limit = 24.0
...
def clear_cam_target_state():
    ...
    global last_follow_mode_key, follow_relock_until_ms
...
def update_follow_targets(gyro_z):
    ...
    if explicit_orbit or explicit_push or explicit_spin:
        follow_relock_until_ms = 0
    elif follow_relock_until_ms and utime.ticks_diff(follow_relock_until_ms, now) <= 0:
        follow_relock_until_ms = 0
    relock_active = follow_relock_until_ms != 0
    if (
        not relock_active
        and last_follow_mode_key > 0
    ...
    if (
        ...
        reset_turn_loop_state()
    angle_pose_mode_active = (not relock_active) and (
        angle_pose_mode_needed(
            cam_error_angle,
            orbit_mode_active,
            spin_mode_active,
    ...
    if seen and not relock_active:
        ...
        if fresh_motion and (not push_follow_active) and (not relock_active):
            if mode_key == 0:
                use_motion_feedforward = (
