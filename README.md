...
`New structure_2/` 是新结构从车调参目录。它从 `NO.2/` 复制从车控制和红外视觉代码，用于在不影响已验证从车版本的前提下重新标定视觉中心、灯点几何和 LSM6DSV16X 陀螺仪调用；当前电机和编码器映射跟随 `New structure/`，红外视觉端使用 `UART(2)`，速度环参数先按主车新结构基准调参。普通跟随在轮速超限时依次保留防撞/基础航向纠偏、主车实测运动、主车预览量和剩余视觉纠偏；明确丢失灯对时短时保持主车实测运动后平滑衰减，相机 UART 超时采用更短的保守过渡。公转保持原有稳态角速度前馈和相对角度闭环，通过延长入口缓启动、限制 yaw 到纵向的补偿及纵向总目标抑制瞬态过冲；退出公转后取消残留运动前馈，以位置纠偏叠加编码器速度阻尼完成联合落位，误差过零后仍主动消除惯性。推行准备和正式推行不再继承公转模式，而是复用普通跟随的前馈、冲突保护、速度阻尼和轮速分配；X/Y 位置纠偏从各自的小窗口起步并独立渐进扩展，单轴过零不影响另一轴，yaw 闭环继续独立锁定，只有轮速实际饱和时才由分配器压缩附加位置纠偏，同时始终保留主车实测速度作为基础前馈。运行日志的目标状态字段使用 `1/0/-1` 分别表示有效灯对、相机明确无灯对和相机通信超时；分配比例为负数时表示主车基础运动也已被轮速上限裁剪。
