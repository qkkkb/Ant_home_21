...
`New structure/` 是新车部署用目录。`main_lsm6dsv16x_95.py` 保留基于 `95eb1b3` 的主控逻辑并适配 LSM6DSV16X 陀螺仪运行时；`move_base_polarity_wireless_test.py` 通过 `C14` 选动作、`C9` 执行动作，配合 `move_base.calc_wheel_spd()` 校验新车电机极性和运动映射，无线串口只做备用控制和纯 ASCII 日志输出。
